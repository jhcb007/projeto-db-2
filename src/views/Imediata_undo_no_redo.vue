                ...
                <div class="md:w-4/12 mb-1 xl:mb-0 px-2">
                  ...
                  <div class="relative flex flex-col min-w-0 break-words bg-white mb-2 shadow-lg rounded">
                    ...
                    <table class="table bg-transparent border-collapse">
                      ...
                      <transition-group :duration="500" enter-active-class="show_td" leave-active-class="end_td"
                                        tag="tbody">
                        <tr v-for="t in transacoes_desfeitas" :key="t.tid+'desf'">
                          <th class="border border-solid border-blueGray-300 border-t-0 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                            <span v-show="t.tid">T{{ t.tid }}</span>
                          </th>
                          <td class="border border-solid border-blueGray-300 border-t-0 text-center align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
...
export default {
  ...
  methods: {
    ...
    abortaTransacao(insert_log = true) {
      ...
      console.log(t_abort(this.operacao));
      //return;
    ...
    falha() {
      ...
      log_disco.forEach(function (l) {
        if (l.operacao === 'checkpoint') {
          return
        }
        const tem_commits = log_disco.filter(f => f.operacao === 'commit' && f.tid === l.tid);
        if (tem_commits.length < 1 && l.operacao === "write_item") {
          operacoes.push(l);
        }
      });
      ...
      operacoes.forEach(function (o) {
        let i = objetos.findIndex(l => l.objeto === o.objeto);
        if (i < 0) {
          objetos.push(o)
        } else {
          objetos[i] = o;
      ...
      this.transacoesDesfeita(objetos);
    ...
    retornaValorAnterior(p) {
      let old = "";
      this.logs.forEach(function (v) {
        if ((v.operacao === 'read_item' || v.operacao === 'write_item') && v.tid === p.transacao.tid && v.objeto === p.objeto.item) {
          old = v.valor;
        }
      });
      return old;
    ...
    transacoesDesfeita(transacoes) {
      ...
      if (this.transacoes_temp.length < 1) {
        return
      }
      ...
      this.transacoes_temp.forEach(function (c) {
        this.transacoes_ativas = this.removeTransacaoAtivas(c.tid);
        this.descartaTransacao(c.tid);
        ...
        this.transacoes_desfeitas.push(t_desfeita(c));

        this.removeTransacao(c.tid);
